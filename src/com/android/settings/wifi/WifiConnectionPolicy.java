...
public class WifiConnectionPolicy extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(mConnectivityManager == null) mConnectivityManager = (ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE);
        ...
        if(WifiManager.WIFI_STATE_CHANGED_ACTION.equals(action)) {
            ...
            if (wifiState == WifiManager.WIFI_STATE_ENABLED) {
                ...
                if (Settings.Global.getInt(context.getContentResolver(), AIRPLANE_MODE_WIFI_NOTIFICATION_FLAG, 0) == 1
                        || Settings.Global.getInt(context.getContentResolver(), Settings.Global.AIRPLANE_MODE_ON, 0) == 0) return;
        ...
        } else if (WifiManager.WIFI_CONNECTED_AP_ABSENT_ACTION.equals(action)) {
            mWlanToWlanDialogCancleFlag = false;
        ...
        else if (WifiManager.WIFI_DISABLED_WHEN_CONNECTED_ACTION.equals(action)) {
            if (!handledRememberedFlag(context, DIALOG_TYPE_WLAN_TO_MOBILE)) {
                mLastWifiInfo = (WifiInfo) intent.getParcelableExtra(WifiManager.EXTRA_WIFI_INFO);
                popUpWlanToMobileDialog(context);
            }
        }
        else if (WifiManager.WIFI_SCAN_RESULT_BSS_REMOVED_ACTION.equals(action)) {
        ...
        else if (action.equals(ALARM_FOR_CONNECT_WIFI_ACTION)) {
            setConnectWifiAlarm(context);
        } else if (action.equals(ALARM_FOR_DISCONNECT_WIFI_ACTION)) {
    ...
    public static void setMobileToWlanPolicy(Context context, int value) {
        Settings.Global.putInt(context.getContentResolver(), Settings.Global.WIFI_MOBILE_TO_WLAN_POLICY, value);
        applyWifiPolicy(context, false);
        resetTimer();
    }

    public static void setManulConnectFlags(boolean flag) {
        logd("setManulConnectFlags flags = " + flag);
        manulConnect = flag;
    }
    public static int getMobileToWlanPolicy(Context context) {
        int value =Settings.Global.getInt(context.getContentResolver(),
               Settings.Global.WIFI_MOBILE_TO_WLAN_POLICY, MOBILE_TO_WLAN_AUTO);
        return value;
    }
    ...
    private static boolean isTimerRunning() {
        ...
        logd("isTimerRunning: " + isTimerRunning);
    ...
    private static void showMobileToWlanDialog(Context context) {
        ...
        if (mobileToWlan == MOBILE_TO_WLAN_AUTO) {
            // For cmcc case 4.7 auto connect trust aps, it's has relationship
            // with auto connect functions
            enableAutoConnect();
        } else {
